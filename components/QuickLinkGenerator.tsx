...
import { createPaymentLink } from '@/utils/api';

export default function QuickLinkGenerator() {
  const [itemName, setItemName] = useState('');
  const [price, setPrice] = useState('');
  const [phone, setPhone] = useState('');
  const [feeSplit, setFeeSplit] = useState<'buyer' | 'seller' | 'split'>('split');
  ...
  const handleGenerate = async () => {
    if (!itemName || !price || !phone) {
      setError('Please provide item name, amount, and phone number');
      return;
    }
    ...
    try {
      ...
      // Use the tracking URL from backend for tracking, but create payment link for sharing
      // The backend returns trackingUrl, but we want to share the payment link
      const paymentLink = `${window.location.origin}/pay/${response.data.trackingId}`;
      setTrackingId(response.data.trackingId);
      setGeneratedLink(paymentLink);
      
      // Log success (in production, this would trigger SMS)
      console.log(`Payment link created. Tracking ID: ${response.data.trackingId}`);
      if (phone) {
        console.log(`SMS would be sent to ${phone}: ${paymentLink}`);
      }
    } catch (err: any) {
  ...
  const handleShareWhatsApp = () => {
    if (generatedLink && itemName) {
      const message = `Pay securely for ${itemName} via VeriPay: ${generatedLink}`;
  ...
  const handleShareInstagram = () => {
    ...
    // We'll copy the link and show a message, or open Instagram
    if (generatedLink) {
      navigator.clipboard.writeText(generatedLink);
    ...
    return (
      <div className="bg-white rounded-2xl shadow-2xl border border-gray-100 p-8 max-w-md w-full">
        ...
        <div className="mb-4">
          ...
          <div className="mb-2">
            <p className="text-xs text-gray-600 mb-3 text-center font-medium">Share on social media</p>
        ...
        <div className="bg-purple-50 border border-purple-200 rounded-lg p-3 flex items-start gap-2">
          <Smartphone className="text-purple-600 flex-shrink-0 mt-0.5" size={16} />
          <p className="text-xs text-purple-900">
            We've sent this link to your phone via SMS so you don't lose it.
          </p>
        </div>
      ...
      <div className="space-y-5">
            ...
            <div className="mt-3 bg-purple-50 border border-purple-200 rounded-lg p-3">
              ...
              <div className="flex justify-between items-center text-xs mb-2">
                <span className="text-gray-500">
                  {calculations.feeSplit === 'buyer' && 'Paid by buyer'}
                  {calculations.feeSplit === 'seller' && 'Paid by you'}
        ...
        <div>
          ...
          <input
            ...
            placeholder="080... (for WhatsApp updates)"
            className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-colors text-gray-900 placeholder-gray-400"
            required
          />
          <p className="text-xs text-gray-500 mt-1">
            We'll send the link to your phone for safekeeping
          </p>
        </div>
        ...
        <button
          ...
          disabled={!itemName || !price || !phone || isGenerating}
        ...
        >
          {isGenerating ? (
            ...
            <>
              Generate Secure Link — It's Free
            </>
          )}
        </button>
      </div>
