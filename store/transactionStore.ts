...
const createDummyData = () => {
  const dummyTrustLinks: TrustLink[] = [
    {
      ...
      description:
        "Brand new iPhone 15 Pro Max 256GB in Natural Titanium. Factory sealed with original accessories. Includes 1 year warranty.",
      imageUrl:
    ...
    },
    {
      ...
      description:
        "MacBook Pro 14-inch with M3 chip, 16GB RAM, 512GB SSD. Excellent condition, barely used. Comes with original box and charger.",
      imageUrl:
    ...
    },
    {
      ...
      description:
        "Premium noise-cancelling wireless headphones. Like new condition with original packaging. Perfect for travel and daily use.",
      imageUrl:
  ...
  const dummyTransactions: Transaction[] = [
    ...
    // Disputed transactions
    {
      ...
      isDisputed: true,
      disputeReason:
        "Item received is different from description. Screen has scratches and battery health is only 65% instead of the advertised 95%.",
      ...
      chatMessages: [
        {
          ...
          sender: "buyer",
          message:
            "The phone I received has significant scratches and the battery is not as described.",
          timestamp: daysAgo(1),
        },
        {
          id: "msg_2",
          sender: "seller",
          message:
            "I shipped the phone in perfect condition. The damage must have occurred during shipping.",
          timestamp: daysAgo(1),
        },
      ],
    },
    {
      ...
      isDisputed: true,
      disputeReason:
        "Item appears to be counterfeit. Stitching quality and logo placement do not match authentic Gucci products.",
      ...
      chatMessages: [
        {
          ...
          sender: "buyer",
          message:
            "I believe this is a fake. The quality is not what I expected from Gucci.",
...
interface TransactionStore {
  ...
  createTrustLink: (
    link: Omit<TrustLink, "id" | "createdAt" | "used">
  ) => string;
  ...
  useTrustLink: (id, transactionId) => {
    set((state) => ({
      trustLinks: state.trustLinks.map((link) =>
        link.id === id
          ? { ...link, used: true, transactionId, status: "used" }
          : link
      ),
    }));
  },

  updateTrustLinkPhone: (id: string, phone: string) => {
    set((state) => ({
      trustLinks: state.trustLinks.map((link) =>
        link.id === id
          ? { ...link, sellerPhone: phone, status: "active" }
          : link
      ),
    }));
  },

  getUnclaimedLinksByPhone: (phone: string) => {
    return get().trustLinks.filter(
      (link) =>
        link.sellerPhone === phone &&
        (link.status === "unclaimed" || link.status === "draft")
    );
  },
  ...
  updateTransactionStage: (id, stage) => {
    set((state) => ({
      transactions: state.transactions.map((txn) =>
        txn.id === id ? { ...txn, stage, updatedAt: new Date() } : txn
      ),
    }));
  },
